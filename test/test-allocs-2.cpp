...
// same as in context, but with heap_stack alloc

TEST_CASE("Context switching basic with heap_stack", "[allocs-2]") {;
    ...
    nothread::execution_context ctx = nothread::call_alloc(nothread::heap_stack(), say_hello, "2", "3", "5");
    v.log("returned");
    ctx = ctx.resume();
    v.log("returned-2");
    ctx = ctx.resume();
...
TEST_CASE("Context switching args with heap_stack", "[allocs]") {;
    ...
    nothread::execution_context ctx = nothread::call_alloc(nothread::heap_stack(4096), say_hello);
